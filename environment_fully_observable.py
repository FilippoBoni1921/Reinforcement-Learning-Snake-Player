...
class BaseEnvironment:
    ...
    def move(self, actions):
        ...
        n_boards_where_fruits_is_been_eaten = len(boards_where_fruits_is_been_eaten)
        ...
        n_boards_where_bodies_is_been_eaten = 0
        if len(bodies_head):
            ...
            n_boards_where_bodies_is_been_eaten = len(eaten_body_peace)
        ...
        return tf.reshape(tf.convert_to_tensor(rewards, dtype=tf.float32), (-1, 1)), n_boards_where_fruits_is_been_eaten, n_boards_where_bodies_is_been_eaten,len(hit_wall),wins
    ...
    def to_state_direction(self,actions):

        boards_with_body = np.unique(np.argwhere(self.boards == 3)[:,0])
        head_positions = np.argwhere(self.boards == 4)

        new_boards = self.boards.copy()

        for i in boards_with_body:

            action = actions[i]

            if action == 0:
                new_boards[i][head_positions[i,1]-1,head_positions[i,2]] = 5
            
            elif action == 1:
                new_boards[i][head_positions[i,1],head_positions[i,2]-1] = 5
        ...
        return K.utils.to_categorical(new_boards)[..., 1:]
